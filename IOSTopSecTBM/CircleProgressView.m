...
- (void)drawArc{
    ...
    if (_percent == 100) {
        ...
        float endAngle = 2*M_PI*_percent/100;
...
- (void)addCenterLabel{
    ...
    if (_percent == 100) {
        percent = @"100%";
        fontSize = 52;
        arcColor = (_arcFinishColor == nil) ? [UIColor redColor] : _arcFinishColor;
    }else if(_percent < 100 && _percent >= 0){
        fontSize = 10;
        arcColor = (_arcUnfinishColor == nil) ? [UIColor redColor] : _arcUnfinishColor;
        percent = [NSString stringWithFormat:@"%0.2f%%",_percent];
        //percent = @"33.1";
    }
    CGSize viewSize = self.bounds.size;
